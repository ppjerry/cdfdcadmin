...
*  房源附属内容处理
*/
class DirectionAction extends CommonAction {
  protected $db;
  ...
  public function add() {
    if (IS_POST) {
      ...
      if ( !empty($pid) ) {
        $parent_toward = $this->db->find($pid);
        $this->assign( 'parent_toward', $parent_toward );
      }
      $this->assign( 'type', isset($parent_toward['belong']) ? $parent_toward['belong'] : 0 );
      $this->assign('types', array( '1' => '新房', '2' => '二手房', '3' => '商铺', '4' => '写字楼', '5' => '别墅' ));
      $towards = $this->db->where( array( 'pid' => 0 ) )->select();
      $this->assign( 'pid', $pid );
      $this->assign( 'towards', $towards );
      $this->display();
    }
