...
* 楼层
*/
class FloorAction extends CommonAction {
  protected $db;
  ...
  public function add() {
    if (IS_POST) {
      ...
      if ( !empty($pid) ) {
        $parent_layer = $this->db->find($pid);
        $this->assign( 'parent_layer', $parent_layer );
      }
      $this->assign( 'type', isset($parent_layer['belong']) ? $parent_layer['belong'] : 0 );
      $this->assign('types', array( '1' => '新房', '2' => '二手房', '3' => '商铺', '4' => '写字楼', '5' => '别墅' ));
      $layers = $this->db->where( array( 'pid' => 0 ) )->select();
      $this->assign( 'pid', $pid );
      $this->assign( 'layers', $layers );
      $this->display();
    }
