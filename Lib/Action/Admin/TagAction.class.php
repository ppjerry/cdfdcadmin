...
class TagAction extends CommonAction {
  ...
  public function index() {
    if (isset( $_GET['type'] )) {
      $specials = $this->db->where( array( 'belong' => array( 'in' , array( 0, intval($_GET['type']) ) ) ) )->order("sort desc")->select();
    } else {
      $specials = $this->db->where( array( 'belong' => 0 ) )->order("sort desc")->select();
    }
  ...
  public function add() {
    if (IS_POST) {
      ...
      if ( !empty($pid) ) {
        $parent_special = $this->db->find($pid);
        $this->assign( 'parent_special', $parent_special );
      }
      $this->assign( 'type', isset($parent_special['belong']) ? $parent_special['belong'] : 0 );
      $this->assign('types', array( '1' => '新房', '2' => '二手房', '3' => '商铺', '4' => '写字楼', '5' => '别墅' ));
      $specials = $this->db->where( array( 'pid' => 0 ) )->select();
      $this->assign( 'pid', $pid );
      $this->assign( 'specials', $specials );
      $this->display();
    }
